...
router.get('/', function(req, res, next){
    try {
        ...
        var tmp = ['about_us', 'about_cn', 'about_hk'];
        WebsiteInfoModel.getInstance().getAll('info_key IN (\'' + tmp.join('\',\'') + '\')').then(function(rows){
            var about = {};
            rows = rows || [];
            rows.forEach(function(item){
                about[item.info_key] = item.content;
            });
            res.doc.about = about;
            res.render('wm/about', res);
        }, function(err){
            next(err);
        });
    } catch (err) {
...
router.get('/:locale', function(req, res, next){
    try {
        ...
        WebsiteInfoModel.getInstance().getOne('info_key=?', ['about_' + locale]).then(function(row){
            var desc = row && row.content;
            res.doc.desc = desc;
            res.render('wm/about_desc', res);
        }, function(err){
            next(err);
        });
    } catch (err) {
...
router.post('/:locale', function(req, res, next){
    try {
        ...
        WebsiteInfoModel.getInstance().writeAboutDesc(content, locale).then(function(){
            res.redirect('/wm/about');
        }, function(err){
            next(err);
        });
    } catch (err) {
